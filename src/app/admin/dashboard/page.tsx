...
export default function AdminDashboard() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Record<ConfigurableColumnKey, boolean>>(getInitialVisibleColumns());
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!token) { setFormError('Token non trovato.'); setFormStatus('error'); return; }
    ...
    try {
      ...
      const payload: any = {
        titolo: formData.titolo, descrizioneBreve: formData.descrizioneBreve, categoria: formData.categoria,
        giocatori: formData.giocatori, durata: formData.durata, difficolta: formData.difficolta, rules: formData.rules,
      };
  ...
  return (
    <div className="flex min-h-screen bg-gray-900 text-gray-100">
      <aside className="w-20 md:w-64 bg-gray-800 p-4 md:p-6 space-y-8 shadow-lg fixed top-0 left-0 h-full z-30 transition-all duration-300 ease-in-out hidden sm:block">
        ...
        <nav className="space-y-3">
          <a
            href="#"
            className="flex items-center space-x-3 p-2 md:p-3 rounded-lg text-gray-300 bg-brand-blue bg-opacity-80 hover:bg-opacity-100 transition-colors"
            aria-current="page"
          >
            <IconCatalog />
          ...
          <a
            href="#"
            className="flex items-center space-x-3 p-2 md:p-3 rounded-lg text-gray-400 hover:text-gray-100 hover:bg-gray-700 transition-colors"
          >
            <IconEvents />
      ...
      <main className="flex-1 sm:ml-20 md:ml-64 p-4 sm:p-6 md:p-10 space-y-8 sm:space-y-10">
        <header className="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
          ...
          <button
            onClick={openNewForm}
            className="w-full sm:w-auto bg-brand-blue hover:bg-opacity-80 text-white font-semibold px-4 py-2 sm:px-6 sm:py-3 rounded-lg shadow-md transition-colors duration-150 ease-in-out flex items-center justify-center space-x-2"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
        ...
        {error && (
          <p className="bg-red-800 text-red-100 p-3 sm:p-4 rounded-md shadow-lg text-sm sm:text-base">
            {error}
          </p>
        )}

        <section className="mt-8 sm:mt-12">
          <div className="mb-6 flex flex-col sm:flex-row sm:justify-end items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-3">
            <div className="relative" ref={columnSelectorRef}>
              <button
                onClick={() => setColumnSelectorOpen(!columnSelectorOpen)}
                className="w-full sm:w-auto bg-gray-700 hover:bg-gray-600 text-gray-300 font-medium px-4 py-2 rounded-lg shadow-md flex items-center justify-center space-x-2 transition-colors"
              >
                <IconColumns />
                <span>Configura Colonne</span>
              </button>
              {columnSelectorOpen && (
                <div className="absolute right-0 mt-2 w-60 bg-gray-700 rounded-lg shadow-xl z-40 p-4 space-y-3 border border-gray-600">
                  ...
                  {configurableColumns.map(col => (
                    <label key={col.key} className="flex items-center space-x-2 cursor-pointer p-1 hover:bg-gray-600 rounded-md">
                      <input
                        ...
                        className="form-checkbox h-4 w-4 text-brand-blue bg-gray-600 border-gray-500 rounded focus:ring-brand-blue focus:ring-opacity-50"
                      />
          ...
          {loadingList ? (
            <p className="text-center text-lg sm:text-xl text-gray-400 py-10">Caricamento...</p>
          ) : games.length === 0 && !error ? (
            <p className="text-center text-lg sm:text-xl text-gray-400 py-10 bg-gray-800 rounded-lg shadow-md">Nessun gioco.</p>
          ) : !error && games.length > 0 ? (
            <div className="bg-gray-800 rounded-xl shadow-2xl overflow-x-auto">
              <table className="w-full min-w-[640px] table-auto">
                <thead className="bg-gray-700">
                  <tr>
                    <th scope="col" className="px-4 py-3 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-200 uppercase tracking-wider">Titolo</th>
                    {configurableColumns.map(col => visibleColumns[col.key] && (
                      <th key={col.key} scope="col" className="px-4 py-3 sm:px-6 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-200 uppercase tracking-wider hidden sm:table-cell">{col.label}</th>
                    ))}
                    <th scope="col" className="px-4 py-3 sm:px-6 sm:py-4 text-center text-xs sm:text-sm font-semibold text-gray-200 uppercase tracking-wider">Azioni</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-700">
                  {games.map((g) => (
                    <tr key={g.documentId || g.id} className="hover:bg-gray-750 transition-colors duration-100 ease-in-out">
                      ...
                      {configurableColumns.map(col => visibleColumns[col.key] && (
                        <td key={col.key} className="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap text-sm text-gray-300 hidden sm:table-cell">{(g as any)[col.key]}</td>
                      ))}
                      <td className="px-4 py-3 sm:px-6 sm:py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div className="flex items-center justify-center space-x-1">
                          <button
                            onClick={() => openEditForm(g)}
                            className="text-indigo-400 hover:text-indigo-300 p-1.5 sm:p-2 rounded-md hover:bg-gray-700 transition-all"
                          ...
                          <button
                            ...
                            className="text-red-500 hover:text-red-400 p-1.5 sm:p-2 rounded-md hover:bg-gray-700 transition-all"
      ...
      {formOpen && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 p-4 transition-opacity duration-300 ease-in-out animate-fadeIn"
        ...
        >
          <div
            ref={modalContentRef}
            className="bg-gray-800 p-4 sm:p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-3xl mx-auto transform transition-all duration-300 ease-in-out scale-95 opacity-0 animate-modalShow"
          >
            <div className="flex justify-between items-center mb-4 sm:mb-6">
              <h2 id="modal-title" className="text-xl sm:text-2xl font-semibold text-white">
                {formData.documentId ? 'Modifica Gioco' : 'Crea Nuovo Gioco'}
              </h2>
              <button
                type="button"
                onClick={() => setFormOpen(false)}
                className="text-gray-400 hover:text-gray-200 transition-colors p-1 rounded-full hover:bg-gray-700 -mr-1 sm:-mr-2"
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                {[
                  ...
                  <label key={name} className="flex flex-col space-y-1">
                    ...
                    <input
                      ...
                      className="p-2 sm:p-3 bg-gray-700 text-gray-100 rounded-md border border-gray-600 focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition-shadow"
                      required={required}
                    />
                  </label>
                ))}
                ...
                <label className="flex flex-col space-y-1 md:col-span-2">
                  ...
                  <textarea
                    name="rules" value={formData.rules} onChange={handleChange} rows={4}
                    className="p-2 sm:p-3 bg-gray-700 text-gray-100 rounded-md border border-gray-600 focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition-shadow"
                  />
                </label>
                <div className="flex flex-col space-y-1">
                  <span className="text-sm font-medium text-gray-300">Immagine Copertina</span>
                  <input type="file" name="coverFile" accept="image/*" onChange={handleFileChange} className="mt-1 block w-full text-xs sm:text-sm text-gray-400 file:mr-2 file:py-1.5 file:px-2 sm:file:mr-4 sm:file:py-2 sm:file:px-4 file:rounded-md file:border-0 file:text-xs sm:file:text-sm file:font-semibold file:bg-brand-blue file:text-white hover:file:bg-opacity-80 cursor-pointer"/>
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <input type="file" name="detailFile" accept="image/*" onChange={handleFileChange} className="mt-1 block w-full text-xs sm:text-sm text-gray-400 file:mr-2 file:py-1.5 file:px-2 sm:file:mr-4 sm:file:py-2 sm:file:px-4 file:rounded-md file:border-0 file:text-xs sm:file:text-sm file:font-semibold file:bg-brand-blue file:text-white hover:file:bg-opacity-80 cursor-pointer"/>
                  {formData.detailFile && <span className="text-xs text-gray-400 mt-1">Selezionato: {formData.detailFile.name}</span>}
                </div>
              </div>
              {formError && <p className="bg-red-900 bg-opacity-50 text-red-300 p-2 sm:p-3 rounded-md text-xs sm:text-sm">{formError}</p>}
              <div className="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 pt-4 border-t border-gray-700 mt-4 sm:mt-6">
                <button
                  type="submit"
                  disabled={formStatus === 'submitting'}
                  className="w-full sm:w-auto bg-brand-yellow hover:bg-opacity-90 text-gray-900 font-bold px-6 py-3 rounded-lg shadow-md transition-colors duration-150 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  type="button"
                  onClick={() => setFormOpen(false)}
                  className="w-full sm:w-auto bg-gray-600 hover:bg-gray-500 text-gray-100 font-semibold px-6 py-3 rounded-lg shadow-md transition-colors duration-150 ease-in-out"
                >
                  Annulla
                </button>
              </div>
